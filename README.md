...
## Fix Large Video Upload (Livewire + XAMPP Apache)

If video uploads fail for large files, apply the settings below.
...
```php
'rules' => ['file', 'max:3145728'], // 3GB in KB
'max_upload_time' => 120, // or higher
```

In `app/Livewire/Admin/Settings.php`, keep:

```php
'videoFile' => 'nullable|mimes:mp4,webm,ogg|max:3145728'
```

### 2) XAMPP PHP limits

Edit your active `php.ini` (usually `C:\xampp\php\php.ini`):
...
- `memory_limit` does not need to match 3GB (512M or 1024M is usually enough).

### 3) Apache limits (if set)

Check Apache config and `.htaccess` for request size limits:

```apache
LimitRequestBody 0
```

`LimitRequestBody` should be unset or `0` (unlimited).  
If `mod_reqtimeout` is enabled, very slow uploads may still fail; increase/relax read timeouts if needed.
