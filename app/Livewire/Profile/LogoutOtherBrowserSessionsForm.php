...
class LogoutOtherBrowserSessionsForm extends Component
{
    ...
    public function logoutOtherBrowserSessions(StatefulGuard $guard)
    {
        if (config('session.driver') !== 'database') {
            $this->dispatch('toastr', ['type' => 'warning', 'message' => 'Session driver must be set to database to use this feature.']);
            return;
        }
        ...
        if (! Hash::check($this->password, Auth::user()->password)) {
            $this->dispatch('toastr', ['type' => 'error', 'message' => 'This password does not match our records.']);
        ...
        $this->dispatch('toastr', ['type' => 'success', 'message' => 'All other browser sessions have been logged out successfully.']);
    }
